...
def _choose_order_size(rng: random.Random, size3_bias: float) -> int:
    ...
    for idx in range(dataset.total_orders):
        ...
        if idx < len(dataset.observed_orders_exact):
            ...
            if rng.random() < cfg.template_repeat_prob:
                ...
                size = _choose_order_size(rng, cfg.size3_bias)
                required = []
                for _ in range(size):
...
def simulate_policy(
    ...
    *,
    ...
    while not game.game_over:
        ...
        collisions_avoided += engine.last_collisions_avoided
        swaps_prevented += engine.last_swaps_prevented
        replans += engine.last_replans
        if engine.last_fallback_used:
            fallback_rounds += 1

        step_info = game.step(actions)
        blocked_moves += int(step_info["blocked_moves"])
...
TUNABLE_SPACE: dict[str, list[Any]] = {
    ...
    "assignment_strategy": ["greedy", "auction"],
...
def main() -> None:
    ...
    if args.mode == "single":
        ...
        result = simulate_policy(dataset, policy, forecast_mode=args.forecast_mode, debug=True)
        print(
