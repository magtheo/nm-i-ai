    ...
    def __post_init__(self):
        ...
        if parent in PHASE_MAPPINGS:
            ...
            if not self.log_file:
                self.log_file = mapping.log_file
...
class Analysis:
    ...
    def bottlenecks(self, threshold_percent: float = 30.0) -> list[Bottleneck]:
        ...
        total_time = sum(t.total for t in parent_phases.values()) if parent_phases else sum(t.total for t in timers.values())
                ...
                if name in grouped and grouped[name]["sub_phases"]:
                    for sub_name, sub_info in grouped[name]["sub_phases"].items():
                        ...
                        sub_phases.append({
                            ...
                            "function": sub_mapping.function_name if sub_mapping else sub_name,
                            "line_hint": sub_mapping.line_hint if sub_mapping else f"def {sub_name}",
                            "log_marker": sub_mapping.log_marker if sub_mapping else f"[PHASE:{name}:{sub_name}]",
                        })
        ...
        for name, timer in timers.items():
            ...
            if ":" in name:
                ...
                all_phases[name] = {
                    ...
                    "avg_ms": timer.avg,
                    "count": timer.count,
                    "log_file": mapping.log_file if mapping else "logs/main.log",
                    "source_file": mapping.source_file if mapping else "unknown",
                    "function": sub_mapping.function_name if sub_mapping else name.split(":")[1],
                    "line_hint": sub_mapping.line_hint if sub_mapping else f"def {name.split(':')[1]}",
                    "log_marker": sub_mapping.log_marker if sub_mapping else f"[PHASE:{name}]",
                    "is_sub_phase": True,
                    "parent": parent,
                }
            else:
    ...
    def print_ai_context(self) -> None:
        ...
        if priority:
            ...
            if priority.get('sub_phase_breakdown') and len(priority['sub_phase_breakdown']) > 0:
                print(f"\nSUB-PHASE TIMING for '{phase}':")
                for sub in priority['sub_phase_breakdown']:
                    marker = " <-- SLOWEST" if sub == priority['slowest_sub_phase'] else ""
    ...
    def print_report(self, include_ai_context: bool = True) -> None:
        ...
        if summary['phases']:
            ...
            for name, stats in sorted(parent_phases.items(), key=lambda x: x[1]['total_ms'], reverse=True):
