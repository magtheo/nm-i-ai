...
from bot.expert_supply_strategy import ExpertSupplyStrategyEngine
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--experimental-dispatch-engine",
        action="store_true",
        help="Use ExperimentalDispatchEngine (global assignment + cooperative routing)",
    )
    parser.add_argument("--debug", action="store_true", help="Enable verbose client/engine logging")
    parser.add_argument(
        "--legacy-expert-decision-engine",
        action="store_true",
        help="Use legacy DecisionEngine for expert mode instead of ExpertSupplyStrategyEngine",
    )
    parser.add_argument(
        "--params-file",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="greedy",
        choices=["greedy", "auction", "hungarian"],
...
async def run_live_once(
    *,
    ...
    use_expert_supply_strategy = False
    if args.orbit_flow_hypothesis:
    ...
    elif bool(getattr(args, "experimental_dispatch_engine", False)):
        raw_params = getattr(args, "_raw_params_file", {})
        exp_cfg = ExperimentalDispatchConfig.from_overrides(raw_params if isinstance(raw_params, dict) else {})
        ...
        use_expert_supply_strategy = (
            str(args.difficulty).strip().lower() == "expert"
            and not bool(getattr(args, "legacy_expert_decision_engine", False))
        )
        if use_expert_supply_strategy:
    ...
    if args.orbit_flow_hypothesis:
        ...
        engine_mode = "experimental_dispatch_engine"
    elif args.orbit_wall:
        planner_name = str(getattr(args, "orbit_planner", "conveyor_v2"))
        engine_mode = f"orbit_wall_{planner_name}"
    elif use_expert_supply_strategy:
        engine_mode = "expert_supply_strategy"
    else:
    ...
    if artifact_dir is not None:
        _write_json(
            artifact_dir / "config.json",
            {
                ...
                "experimental_dispatch_engine": bool(getattr(args, "experimental_dispatch_engine", False)),
                "legacy_expert_decision_engine": bool(getattr(args, "legacy_expert_decision_engine", False)),
        ...
        (artifact_dir / "log.txt").write_text(
            "\n".join(
                [
                    ...
                    f"experimental_dispatch_engine={getattr(args, 'experimental_dispatch_engine', False)}",
                    f"legacy_expert_decision_engine={getattr(args, 'legacy_expert_decision_engine', False)}",
...
async def async_main() -> None:
    ...
    if bool(getattr(args, "experimental_dispatch_engine", False)) and (args.orbit_wall or args.orbit_flow_hypothesis):
        raise SystemExit("Choose only one experimental engine: orbit or --experimental-dispatch-engine")
    if str(args.difficulty).strip().lower() == "expert" and (args.orbit_wall or args.orbit_flow_hypothesis):
        print("[info] expert mode with orbit engine is diagnostic/experimental; ExpertSupplyStrategyEngine is the primary expert path.")
