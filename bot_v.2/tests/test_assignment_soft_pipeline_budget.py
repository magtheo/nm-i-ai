...
def test_soft_pipeline_budget_preserves_secure_preview_work() -> None:
    ...
    soft = assign_bots(
        ...
        grid,
        policy=_base_policy(
            soft_pipeline_budget_enabled=True,
            soft_pipeline_secure_delivered_deficit_threshold=2,
        ),
    )

    baseline_pre = sum(1 for assign in baseline.values() if assign.target_type == "pre_pick")
    hard_pre = sum(1 for assign in hard.values() if assign.target_type == "pre_pick")
    soft_pre = sum(1 for assign in soft.values() if assign.target_type == "pre_pick")

    assert baseline_pre >= 2
    assert hard_pre == 1
    assert soft_pre >= baseline_pre - 1
    assert any("soft_pipeline_secure" in str(assign.source) for assign in soft.values())


def test_soft_pipeline_budget_tags_build_mode_sources() -> None:
    ...
    assert any("soft_pipeline_build" in str(assign.source) for assign in soft.values())
