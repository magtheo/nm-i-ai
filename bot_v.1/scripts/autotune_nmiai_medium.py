...
BEST_CONFIG_PATH = BOT_ROOT / "app" / "integrations" / "nmiai_grocery_bot" / "best_configs" / "medium.json"
...
def _run_once(config: CandidateConfig, *, cooldown_sec: float) -> AttemptResult:
...
def main() -> None:
    ...
    baseline_result = _run_once(baseline, cooldown_sec=args.cooldown_sec)
    ...
    for idx, config in enumerate(candidates[1:], start=2):
        ...
        result = _run_once(config, cooldown_sec=args.cooldown_sec)
